...
:- dynamic location/2.
%Q2,Q3- doll, plate in room attic. teleporter in desk.
location(doll, attic).
...
%Q6. function turn_off implemented here 
turn_off(light):-
  respond(['You can''t reach the switch and there''s nothing to stand on']).
turn_off(Thing):-
  have(Thing),
...
can_take(Thing) :-
  ...
  notHeavy(Thing).				%Q4cannot take heavy objects
can_take(Thing) :-
  here(Place),
  location(Thing,X),
  location(X,Place),
...
can_take(Thing) :-
  ...
  notHeavy(Thing).
can_take(Thing) :-
  can_take_twice(Thing).

%Q7
can_take_twice(Thing) :-
  have(Thing),
  write('You already have '),		%Q7. cannot take a thing you already have
  write(Thing),
  write('.'),
  nl.
can_take_twice(Thing) :-
  ...
  not((have(Thing))),
  write(Thing),
  write(' cannot be taken as it is too heavy.'),			%Q4. cannot take a heavy thing
  nl,
  fail.
...
%9. Extra feature implemented. This feature 'has_food' checks if there is anything edible in the room which you are in.
has_food():-
	here(Place),
	edible(X),
	location(X,Place),		%true when something edible exists in the room
	write('This room '),
	write(Place),
	write(' has food.'),
	nl.
has_food():-
	here(Place),
	edible(X),
	not((location(X,Place))),	%true when nothing edible exists in the room.
	write('This room '),
	write(Place),
	write(' has no food.'),
	nl.
...
%Q5- function teleport
teleport(Place):- have(teleporter), move(Place), look, !.		%if you have a teleporter move to 'Place' and look.
teleport(Place):- write('Cannot teleport without a teleporter.'),nl.



toss(Thing):-		%Q8. Tossing a thing that is tossable when in a room that has windows. Tossable items and rooms with windows are defined above.
  have(Thing),
  tossable(Thing),
  here(Place),
  hasWindow(Place),
  retract(have(Thing)),
  write(Thing),
  write(' has been tossed'),
  nl.
toss(Thing):-			% Cannot toss what you don''t have. 
  not((have(Thing))),
  write('Cannot toss what you don''t have.'),
  nl.
toss(Thing):-			%Cannot toss a non-tossable item.
  have(Thing),
  not((tossable(Thing))),
  write('The item '),
  write(Thing),
  ...
  nl.
toss(Thing):-			%Cannot toss if the room has no windows
  here(Place),
  not((hasWindow(Place))),
  write('The item '),
  write(Thing),
  write(' cannot be tossed as the room '),
  write(Place),
  write(' doesn''t have windows.'),
  nl.
...
verb(go_place,teleport) --> [teleport].
